  ...
  <div class="avatar-container">
    ...
    <div class="interaction-panel">
      ...
      <div class="input-area">
        <input v-model="userInput" @keyup.enter="submit" placeholder="Transmit directive..." :disabled="!canInteract" />
...
watch(() => personaStore.responseTrigger, (newValue, oldValue) => {
  ...
  if (commands[0].type === 'error') {
    ...
    orchestrateSpeech(personaStore.responseText, commands);
  }
  isThinking.value = false;
});
...
async function orchestrateSpeech(text, animationCommands) {
  isSpeaking.value = true;
  setParameterValue('ParamAngleX', 0.0);
  setParameterValue('ParamAngleZ', 0.0);
  setParameterValue('PARAM_ANGLE_X', 0.0);
  setParameterValue('PARAM_ANGLE_Z', 0.0);
  console.log("[Orchestrate] Face-forward reset applied");
  const audio = await getAudio(text);
  const duration = audio?.durationSec ? audio.durationSec * 1000 : 5000;
  console.log(`[Orchestrate] Duration: ${duration}ms, Commands: ${animationCommands.length}`);
  if (audio) {
    console.log("[Orchestrate] Playing audio");
    audio.play();
  }
  executeCommandList(animationCommands, duration);
...
onMounted(async () => {
  ...
  try {
    ...
    live2dModel.y = app.screen.height * 2.0;
